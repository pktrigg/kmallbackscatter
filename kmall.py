...
def getruntime(filename):
    ...
    return fmmode
###############################################################################
def loadbackscatterdata(filename, args):
    ...
    while r.moreData():
        ...
        if typeofdatagram == '#MRZ':
            datagram.read()
            #we need to extract the take off angle, backscatter and sector number so do it here.
            ...
            update_progress("Extracting Backscatter", pingcounter/recordcount)
            pingcounter = pingcounter + 1

        if pingcounter == maxpings:
            break

    report["pingcounter"] = pingcounter
    report["beamcounter"] = beamcounter

    #now we have all the data, we can bin the data into angle-sized buckets
    resolution = 1
...
###############################################################################
def computebackscatterpointcloud(datagram, geo, beamcounter):
    ...
    def getruntime(self):
        ...
        while self.moreData():
            ...
            if typeofdatagram == '#IOP':
                datagram.read()
                self.rewind()
                return datagram.runtimeparameters
...
class RANGEDEPTH:
    ...
    def read(self, headeronly=False):
        ...
        self.depthmode                                    = s[4]
        self.depthmodename = "unknown"
        if (self.depthmode == 0 or self.depthmode == 100):
            self.depthmodename = "Very Shallow"
        if (self.depthmode == 1 or self.depthmode == 101):
            self.depthmodename = "Shallow"
        if (self.depthmode == 2 or self.depthmode == 102):
            self.depthmodename = "Medium"
        if (self.depthmode == 3 or self.depthmode == 103):
            self.depthmodename = "Deep"
        if (self.depthmode == 4 or self.depthmode == 104):
            self.depthmodename = "Deeper"
        if (self.depthmode == 5 or self.depthmode == 105):
            self.depthmodename = "Very Deep"
        if (self.depthmode == 6 or self.depthmode == 106):
            self.depthmodename = "Extra Deep"
        if (self.depthmode == 7 or self.depthmode == 107):
            self.depthmodename = "Extreme Deep"
        ...
        for i in range(self.numTxSectors):
            ...
            sector = ggmbes.GGSECTOR()

            if self.version    == 1:
                sector.txSectorNumb            = s[0]
                sector.txArrNumber            = s[1]
                sector.txSubArray                = s[2]
                sector.padding0                = s[3]
                sector.sectorTransmitDelay_sec= s[4]
                sector.tiltAngleReTx_deg        = s[5]
                sector.txNominalSourceLevel_dB= s[6]
                sector.txFocusRange_m            = s[7]
                sector.centreFreq_Hz            = s[8]
                sector.signalBandWidth_Hz        = s[9]
                sector.totalSignalLength_sec    = s[10]
                sector.pulseShading            = s[11]
                sector.signalWaveForm            = s[12]
                sector.padding1                = s[13]
                ...
                sector.signalWaveForm            = s[12]
                sector.padding1                = s[13]
                sector.highVoltageLevel_dB    = s[14]
                sector.sectorTrackingCorr_dB    = s[15]
                sector.effectiveSignalLength_sec    = s[16]
...
class IOP_RUNTIME:
    ...
    def read(self):
        ...
        self.txt = self.fileptr.read(self.numBytesCmnPart)# read the record from disc
        bytesRead = bytesRead + self.numBytesCmnPart
        self.runtime = self.txt.decode('utf-8', errors="ignore")
        ...
        for parameter in parameters:
            if ":" in parameter:
                words = re.split(':',parameter)
                self.runtimeparameters[words[0].strip()] = words[1].strip()
        # for p in parameters:
        # parts = re.split(',',parameters) ; 
        # for p in parts:
        #     p = p.replace("\n","")
